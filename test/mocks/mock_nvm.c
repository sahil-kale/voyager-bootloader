...
voyager_bootloader_nvm_write(const voyager_nvm_key_E key,
  ...
  case VOYAGER_NVM_KEY_APP_SIZE: {
    mock_nvm_data.app_size = *data;
  } break;
  case VOYAGER_NVM_KEY_APP_CRC: {
    mock_nvm_data.app_crc = *data;
  } break;
  case VOYAGER_NVM_KEY_APP_START_ADDRESS: {
    mock_nvm_data.app_start_address = *data;
  } break;
  case VOYAGER_NVM_KEY_APP_END_ADDRESS: {
    mock_nvm_data.app_end_address = *data;
  } break;
  case VOYAGER_NVM_KEY_VERIFY_FLASH_BEFORE_JUMPING: {
    mock_nvm_data.verify_flash_before_jumping = *data;
  } break;
  case VOYAGER_NVM_KEY_APP_RESET_VECTOR_ADDRESS: {
    mock_nvm_data.app_reset_vector_address = *data;
  } break;
  default: {
...
voyager_bootloader_nvm_read(const voyager_nvm_key_E key,
  ...
  switch (key) {
  case VOYAGER_NVM_KEY_APP_SIZE: {
    *data = mock_nvm_data.app_size;
  } break;
  case VOYAGER_NVM_KEY_APP_CRC: {
    *data = mock_nvm_data.app_crc;
  } break;
  case VOYAGER_NVM_KEY_APP_START_ADDRESS: {
    *data = mock_nvm_data.app_start_address;
  } break;
  case VOYAGER_NVM_KEY_APP_END_ADDRESS: {
    *data = mock_nvm_data.app_end_address;
  } break;
  case VOYAGER_NVM_KEY_VERIFY_FLASH_BEFORE_JUMPING: {
    *data = mock_nvm_data.verify_flash_before_jumping;
  } break;
  case VOYAGER_NVM_KEY_APP_RESET_VECTOR_ADDRESS: {
    *data = mock_nvm_data.app_reset_vector_address;
  } break;
  default: {
    error = VOYAGER_ERROR_INVALID_ARGUMENT;
  } break;
